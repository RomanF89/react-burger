...
export const FeedCard: React.FC<TFeedCardProps> = ({ cardStyle, cardClick, item }) => {

  const isOrdersPage = cardStyle === "orders_page" ? true : null;
  const data = useSelector(state => state.ingredients.ingredientsFromRequest);

  //Проверка каждого ингердиента и создание массива
  let currentIngredients: (TIngredient | undefined)[] = [];
  const isFilterIngredients = item.ingredients.length > 1 && item.ingredients.every((items) => items !== null && items !== undefined);
  if (isFilterIngredients) {
